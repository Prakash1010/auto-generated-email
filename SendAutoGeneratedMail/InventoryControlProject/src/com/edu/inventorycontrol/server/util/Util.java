...
public class Util {
	...
	public static Entity findEntity(String kind, String searchBy,
			String searchFor) {
	...
	public static Entity findSingleEntity(String kind, String searchBy,
			String searchFor) {
		logger.log(Level.INFO, "Search single entity based on search criteria");
		Query query = new Query(kind);
		if (searchFor != null && !"".equals(searchFor)) {
	...
	// Projection
	public static Iterable<Entity> fetchPrporties(String kind, String property,
			boolean distinct) {
	...
	public static Iterable<Entity> fetchFilteredData(String kind, Key ancestor,
			Filter filter) {
		logger.log(Level.INFO,
				"Getting Filtered Entities list based on search criteria");
		Query query = new Query(kind);
		if (ancestor != null) {
	...
	public static Iterable<Entity> listEntities(String kind, String searchBy,
		...
		if (searchFor != null && !"".equals(searchFor)) {
			Filter filter = new FilterPredicate(searchBy, FilterOperator.EQUAL,
					searchFor);
			query.setFilter(filter);
		}
		PreparedQuery pq = datastore.prepare(query);
	...
	/**
	 * Get the specified(with search criteria) list of children from a parent
	 * key in the entity group
	 * 
	 * @param kind
	 *            : the entity kind of the children that is to be searched for
	 * @param ancestor
	 ...
	 */
	@SuppressWarnings("deprecation")
	public static Iterable<Entity> listChildEntities(String kind,
			String searchBy, String searchFor, Key ancestor) {
	...
	public static Iterable<Entity> listFilterChildren(String kind,
			Key ancestor, String filter) {
		System.out.println("inside list children.....@@@@");
		logger.log(Level.INFO, "Search entities based on parent");
		Query query = new Query(kind);
		query.setAncestor(ancestor);
		query.addFilter(Entity.KEY_RESERVED_PROPERTY,
				FilterOperator.GREATER_THAN, ancestor).addFilter("Parent_Id",
				FilterOperator.EQUAL, filter);
		// System.out.println("inside listChildren..."+datastore);
